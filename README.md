...
先由你选择 A（参与已有 Issue）、B（主动发现新问题）或 A+B；已给定问题时直接进入对应阶段。B 先读官方贡献要求，再使用多个 subagent 分工探索。深入调查前、推荐前和正式修复前核实认领及已修复状态。

路线确定后提供仓库评估、贡献规则和最多 5 个推荐候选，每个说明理由、参与状态及最后检查时间，由你选择后推进。不足 5 个照实提供，没有合适的就说明。5 个只限制最终推荐数量，不限制信息源或线索数量。自发现 Bug 尽量先最小复现和查重，再按项目规则提交 Issue、确认方向、修复并关联 PR。选定问题后可以继续：
